...
@Evolving
class DescribeTransactionsResult internal constructor(
    private val futures: Map<CoordinatorKey, KafkaFuture<TransactionDescription>>,
) {
     ...
     * respective call to [Admin.describeTransactions].
     */
    fun description(transactionalId: String): KafkaFuture<TransactionDescription> {
        val key = CoordinatorKey.byTransactionalId(transactionalId)
        return requireNotNull(futures[key]) {
     ...
     * call to [Admin.describeTransactions].
     *
     ...
     */
    fun all(): KafkaFuture<Map<String, TransactionDescription>> {
        return KafkaFuture.allOf(*futures.values.toTypedArray()).thenApply {
            futures.map { (key, value) ->
                try {
                    key.idValue to value.get()
