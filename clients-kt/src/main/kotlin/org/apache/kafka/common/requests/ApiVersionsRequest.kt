/*
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements. See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License. You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.apache.kafka.common.requests

import org.apache.kafka.common.message.ApiVersionsRequestData
import org.apache.kafka.common.message.ApiVersionsResponseData
import org.apache.kafka.common.message.ApiVersionsResponseData.ApiVersionCollection
import org.apache.kafka.common.protocol.ApiKeys
import org.apache.kafka.common.protocol.ByteBufferAccessor
import org.apache.kafka.common.protocol.Errors
import org.apache.kafka.common.utils.AppInfoParser.version
import java.nio.ByteBuffer
import java.util.regex.Pattern

// Unlike other request types, the broker handles ApiVersion requests with higher versions than
// supported. It does so by treating the request as if it were v0 and returns a response using
// the v0 response schema. The reason for this is that the client does not yet know what versions
// a broker supports when this request is sent, so instead of assuming the lowest supported version,
// it can use the most recent version and only fallback to the old version when necessary.
class ApiVersionsRequest(
    private val data: ApiVersionsRequestData,
    version: Short,
    private val unsupportedRequestVersion: Short? = null
) : AbstractRequest(ApiKeys.API_VERSIONS, version) {

    fun hasUnsupportedRequestVersion(): Boolean = unsupportedRequestVersion != null

    val isValid: Boolean
        get() = if (version >= 3)
            SOFTWARE_NAME_VERSION_PATTERN.matcher(data.clientSoftwareName).matches() &&
                    SOFTWARE_NAME_VERSION_PATTERN.matcher(data.clientSoftwareVersion).matches()
        else true

    override fun data(): ApiVersionsRequestData = data

    override fun getErrorResponse(throttleTimeMs: Int, e: Throwable): ApiVersionsResponse {
        val data = ApiVersionsResponseData().setErrorCode(Errors.forException(e).code)
        if (version >= 1) data.setThrottleTimeMs(throttleTimeMs)

        // Starting from Apache Kafka 2.4 (KIP-511), ApiKeys field is populated with the supported
        // versions of the ApiVersionsRequest when an UNSUPPORTED_VERSION error is returned.
        if (Errors.forException(e) === Errors.UNSUPPORTED_VERSION) {
            val apiKeys = ApiVersionCollection()
            apiKeys.add(ApiVersionsResponse.toApiVersion(ApiKeys.API_VERSIONS))
            data.setApiKeys(apiKeys)
        }

        return ApiVersionsResponse(data)
    }

    class Builder : AbstractRequest.Builder<ApiVersionsRequest> {
        constructor() : super(ApiKeys.API_VERSIONS)
        constructor(version: Short) : super(ApiKeys.API_VERSIONS, version)

        override fun build(version: Short): ApiVersionsRequest = ApiVersionsRequest(DATA, version)

        override fun toString(): String = DATA.toString()

        companion object {

            private const val DEFAULT_CLIENT_SOFTWARE_NAME = "apache-kafka-java"

            private val DATA = ApiVersionsRequestData()
                .setClientSoftwareName(DEFAULT_CLIENT_SOFTWARE_NAME)
                .setClientSoftwareVersion(version)
        }
    }

    companion object {

        private val SOFTWARE_NAME_VERSION_PATTERN =
            Pattern.compile("[a-zA-Z0-9](?:[a-zA-Z0-9\\-.]*[a-zA-Z0-9])?")

        fun parse(buffer: ByteBuffer, version: Short): ApiVersionsRequest =
            ApiVersionsRequest(ApiVersionsRequestData(ByteBufferAccessor(buffer), version), version)
    }
}
