...
/**
 * Authentication callback for SASL/PLAIN authentication. Callback handler must set authenticated
 * flag to true if the client provided password in the callback matches the expected password.
 *
 * @Constructor Creates a callback with the password provided by the client.
 * @param password The password provided by the client during SASL/PLAIN authentication
 */
class PlainAuthenticateCallback(val password: CharArray) : Callback {

    /**
     * Authenticated state that is `true` if client password matches expected password, `false`
    ...
    fun password(): CharArray = password
    ...
    @Deprecated(
        message = "Use property instead",
        replaceWith = ReplaceWith("authenticated"),
    )
    fun authenticated(): Boolean = authenticated
    ...
    fun authenticated(authenticated: Boolean) {
