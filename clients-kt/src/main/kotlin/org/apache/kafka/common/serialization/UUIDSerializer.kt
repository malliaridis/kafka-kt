...
/**
 * We are converting UUID to String before serializing. String encoding defaults to UTF8 and can be
...
class UUIDSerializer : Serializer<UUID> {
    ...
    override fun serialize(topic: String, data: UUID?): ByteArray? {
        return try {
            data?.toString()?.toByteArray(charset(encoding))
        } catch (e: UnsupportedCharsetException) {
            throw SerializationException("Error when serializing UUID to byte[] due to unsupported encoding $encoding")
        }
    }
}
