...
class OAuthBearerScopeUtilsTest {
    ...
    @Test
    fun invalidScope() {
        arrayOf("\"foo", "\\foo").forEach { invalidScope ->
            assertFailsWith<OAuthBearerConfigException>(
                message = "did not detect invalid scope: $invalidScope",
            ) { parseScope(invalidScope) }
        }
    }
}
