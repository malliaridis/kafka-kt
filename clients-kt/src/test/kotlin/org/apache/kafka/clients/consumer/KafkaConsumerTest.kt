...
class KafkaConsumerTest {
    ...
    fun testPollReturnsRecords() {
        val consumer = setUpConsumerWithRecordsToPoll(tp0, 5)
        val records = consumer.poll(Duration.ZERO)
        assertEquals(records.count(), 5)
        assertEquals(records.partitions(), setOf(tp0))
    ...
    fun testSecondPollWithDeserializationErrorThrowsRecordDeserializationException() {
        ...
        val consumer = setUpConsumerWithRecordsToPoll(
            tp = tp0,
            recordCount = 5,
    ...
    fun testSubscription() {
        val consumer = newConsumer(groupId)
...
//        }
    ...
    fun testSubscriptionWithEmptyPartitionAssignment() {
        ...
        newConsumer(props).use { consumer ->
...
//        }
    ...
    fun testInterceptorConstructorConfigurationWithExceptionShouldCloseRemainingInstances() {
        ...
        try {
            val props = Properties().apply {
                setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9999")
                setProperty(
                    ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, MockConsumerInterceptor::class.java.getName() + ", "
    ...
    fun testFetchProgressWithMissingPartitionPosition() {
        ...
        val node = metadata.fetch().nodes[0]
        val consumer = newConsumerNoAutoCommit(time, client, subscription, metadata)
    ...
    private fun setupThrowableConsumer(): KafkaConsumer<String, String> {
        val offset1: Long = 10000
        val metadata = createMetadata(subscription)
        val client = MockClient(time, metadata)
        initMetadata(client, mapOf(topic to 2))
    ...
    fun testPollWithNoSubscription() {
        newConsumer(null as String?).use { consumer ->
    ...
    fun testGracefulClose() {
        ...
        consumerCloseTest(
            closeTimeoutMs = 5000,
            responses = listOf(commitResponse, leaveGroupResponse),
            waitMs = 0,
            interrupt = false,
        )
    }
    ...
    fun testMetricConfigRecordingLevel() {
        ...
        KafkaConsumer(
            ...
            assertEquals(
                ...
                actual = consumer.metrics.config.recordingLevel,
            )
        }
    ...
    private fun consumerCloseTest(
        ...
        try {
            ...
            for (i in responses.indices) {
                ...
                client.respondFrom(response = responses[i], node = coordinator)
                if (i != responses.size - 1) assertFailsWith<TimeoutException>(
                    message = "Close completed without waiting for response",
                ) { future[100, TimeUnit.MILLISECONDS] }
            }
            ...
            if (interrupt) {
                ...
                future[500, TimeUnit.MILLISECONDS] // Should succeed without TimeoutException or ExecutionException
    ...
    @Test
    fun testListOffsetShouldUpateSubscriptions() {
        val metadata = createMetadata(subscription)
        val client = MockClient(time, metadata)
        initMetadata(client, mapOf(topic to 1))
        val consumer = newConsumer(
    ...
    private fun getConsumerRebalanceListener(consumer: KafkaConsumer<String, String>): ConsumerRebalanceListener {
    ...
    private fun newConsumer(
        ...
        val fetcher = Fetcher(
            ...
            subscriptions = subscription,
            metrics = metrics,
            metricsRegistry = metricsRegistry.fetcherMetrics,
            time = time,
            retryBackoffMs = retryBackoffMs,
