...
class HeaderGenerator(private val packageName: String) {
    ...
    fun generate() {
        ...
        for (i in HEADER.indices) buffer.printf("%s%n", HEADER[i])

        buffer.printf("package %s;%n", packageName)
        buffer.printf("%n")
        for (newImport in imports) buffer.printf("import %s;%n", newImport)

        buffer.printf("%n")
        if (!staticImports.isEmpty()) {
            for (newImport in staticImports) buffer.printf("import static %s;%n", newImport)
            buffer.printf("%n")
        }
    }
