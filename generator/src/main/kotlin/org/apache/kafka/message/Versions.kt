...
class Versions {
    ...
    @Deprecated(
        ...
        replaceWith = ReplaceWith("lowest"),
    )
    fun lowest(): Short = lowest

    @Deprecated(
        message = "User property instead",
        replaceWith = ReplaceWith("highest"),
    )
    fun highest(): Short = highest

    fun empty(): Boolean = lowest > highest

    override fun toString(): String = if (empty()) NONE_STRING
