...
object MessageGenerator {
    ...
    const val JAVA_SUFFIX = ".java"

    const val API_MESSAGE_TYPE_JAVA = "ApiMessageType.java"

    const val API_SCOPE_JAVA = "ApiScope.java"

    const val METADATA_RECORD_TYPE_JAVA = "MetadataRecordType.java"

    const val METADATA_JSON_CONVERTERS_JAVA = "MetadataJsonConverters.java"
    ...
    const val ITERATOR_CLASS = "java.util.Iterator"
        ...
        Files.newDirectoryStream(Paths.get(inputDir), JSON_GLOB).use { directoryStream ->
            for (inputPath: Path in directoryStream) try {
                ...
                for (generator: MessageClassGenerator in generators) {
                    val name = generator.outputName(spec) + JAVA_SUFFIX
                ...
                throw RuntimeException(
                    "Exception while processing $inputPath",
                    e,
                )
            }
        }
