...
internal class SchemaGenerator(
    ...
    fun generateSchemas(
        ...
        for (v in versions.lowest..versions.highest) {
            ...
            if (cur != prev) {
                messageInfo.schemaForVersion[v.toShort()] = cur
            }
            prev = cur
        }
    }
    ...
    private fun fieldTypeToSchemaType(
        ...
        return if (type is BoolFieldType) {
            ...
            if (fieldFlexibleVersions.contains(version)) {
                ...
                val prefix = if (nullable) "CompactArrayOf.nullable" else "new CompactArrayOf"
                String.format(
                    "%s(%s)", prefix,
                ...
                val arrayType = type as FieldType.ArrayType
                val prefix = if (nullable) "ArrayOf.nullable" else "new ArrayOf"
                String.format(
                    "%s(%s)", prefix,
                    fieldTypeToSchemaType(
    ...
    fun writeSchema(className: String, buffer: CodeBuffer) {
        ...
        buffer.printf("};%n")
        buffer.printf("%n")
        buffer.printf("public static final short LOWEST_SUPPORTED_VERSION = %d;%n", lowest)
        buffer.printf("public static final short HIGHEST_SUPPORTED_VERSION = %d;%n", highest)
        buffer.printf("%n")
    }
}
